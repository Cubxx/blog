大家好哇，我叫肖宜清，是上海师范大学心理学院 2020 级本科生。

我打算针对科研工作者构建一个产品生态，于是我发起了 Lazybones 项目。
这个项目是非营利性的，非营利不是指无法盈利，而是不以盈利为目标。
Lazybones 的目标是消除科研工作中的一切重复劳动，把时间更多地花在学术思考中。
...
-   [当前科研工作流](#当前科研工作流)
    ...
    -   [数据分析](#数据分析)
    -   [撰写文字](#撰写文字)
-   [Lazybones 产品介绍](#lazybones-产品介绍)
    -   [招募被试：Essai](#招募被试essai)
    -   [数据分析](#数据分析-1)
    -   [撰写文字：wps-paper](#撰写文字wps-paper)
    -   [排版系统](#排版系统)
-   [展望](#展望)
-   [加入](#加入)
-   [捐助](#捐助)
...
详见[Lazybones 产品介绍](#lazybones-产品介绍)。
...
各种各样的研究目的和数据处理软件使得数据分析可能很难有统一的流程。
于是我们计划构建一个完整的方案，将数据分析的不同步骤连接在一起，
这样就不用在电脑上打开各种各样的窗口了。

## 撰写文字

大多研究员使用 Excel 作图、用 Word 排版。这些纯手工操作需要浪费大量时间，尤其是 Word 排版。
理工科常用的排版系统是 Latex，但是 Latex 语法繁琐，学习成本较高，还需要安装相应的编译器。
目前市面上也没有更简单的排版系统了，即使有也无法导出为 Word 文档。
...
3. Python 是开源的，这使得它有更多的网络资料方便学习。
4. Python 生态丰富，这是 Python 成为热门语言的一个很重要的原因。
   从数据爬虫到人工智能，你几乎可以用 Python 做任何事。

在一些细分领域，Python 可能还无法替代其他软件。
所以这个库还提供了与其他软件通信的功能，即通过 Python 调用其他软件，并获取数据处理结果。

## 撰写文字：[wps-paper](https://github.com/cubxx/wps-paper)

这是一个 wps 加载项，正在维护开发中。
它已经实现了自动作表、撰写结果部分，但是它需要 Python 环境才能安装。

## 排版系统

这是一个线上程序，运行在浏览器里。
它可以将简单语法转化成规范的文档，就像 typst 一样：

![](pic/typst.png)

但是这个排版系统的主要优势是可以导出为 Word 文档，以兼容现有的工作流。
这个排版系统还支持自定义样式以覆盖默认样式，方便应对各种各样的排版格式要求。
